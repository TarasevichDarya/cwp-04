...
const resBad = 'DEC';
const resFiles = 'NEXT';
const defaultDir = process.env.VSCODE_CWD;
    ...
    client.on('data', (data) => {
        ...
        if (clients[client.id] === reqRemote && data !== reqRemote) {
            ...
            if (remoting[0] === reqRemCopy) {
                rd.pipe(wr).on('close', sendNext);                // re.pipe(destintaion[,options])
                                                                  //
                
            }
            if (remoting[0] === reqRemEncode) {
                let cryptoStream = crypto.createCipher(algorithm, remoting[3]);  //(alg, password[, options])
